...
export const BuyButton: React.FC<BuyButtonProps> = ({
  ...
  if (isProductInCart(product.id)) {
    return (
      <div className={styles.wrapper}>
        <button className={styles['buy-button']} onClick={onDecreaseAmount}>
          -
        </button>
        <span className={styles['buy-button__amount']}>
          {getProductAmountInCart(product.id)}
        </span>
        <button className={styles['buy-button']} onClick={onIncreaseAmount}>
          +
        </button>
      </div>
    )
  ...
  return (
    <button onClick={onAddToCart} className={styles['buy-button']}>
      COMPRAR
    </button>
  )
}
